...
import { InfoCalorias } from "../components/InfoCalorias"
import TablaNutricion from "../components/TablaNutricion"
import { UserContext } from "../context/UserContext";
import { useDispatch, useSelector } from "react-redux";
import { getProducts } from "../actions/userActions";
import { GET_PRODUCTS_SUCCESS } from "../constants/userConstants";
...
export const NutritionPage = () => {
  const { productos, setProductos, caloriasRestantes, setCaloriasRestantes, caloriasConsumidas,
    ...
    , totalDesayuno, totalAlmuerzo, totalComida, totalMerienda, totalCena } = useContext(UserContext);

  const dispatch = useDispatch();
  const getProducts2 = useSelector((state) => state.getProducts);
  const { GetProductsError, GetProductsLoading, GetProductsInfo, GetProductsType } = getProducts2;
  ...
  if (GetProductsType == GET_PRODUCTS_SUCCESS) {
    setProductos(GetProductsInfo.productos);
  }
  return (
    <div className="">
      <Diary />
